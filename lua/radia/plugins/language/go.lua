return {
	...
	config = function()
		local iferr_helper = vim.api.nvim_get_runtime_file("lua/go/iferr_highlight.lua", false)[1]
		if not iferr_helper then
			vim.schedule(function()
				vim.notify(
					"[go.nvim] Disabled: installed plugin is missing lua/go/iferr_highlight.lua. Reinstall go.nvim to restore Go features.",
					vim.log.levels.WARN
				)
			end)
			return
		...
		if not ok then
			vim.schedule(function()
				vim.notify(("[go.nvim] Disabled because setup failed.\n%s"):format(tostring(err)), vim.log.levels.WARN)
			end)
		end
	end,
	event = { "CmdlineEnter" },
